...
public interface SectionRepository extends JpaRepository<Section,Long> {
	...
	@Query(nativeQuery = true, value = "select sec.category, bs.current_quantity, s.price, w.ware_house_name, p.name  from ware_house w, section sec, inbound_order i, batch_stock bs, sales_ad s,  products p where s.idproduct = p.idproduct and bs.idsales_ad = s.idsales_ad and i.idbatch_number = bs.idbatch_number and i.section_code = sec.section_code and sec.id_ware_house = w.id_ware_house and sec.category like %?1%")
	List<SectionByProducts> CategoryContaining(String name);
